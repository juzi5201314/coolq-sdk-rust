...
pub(crate) fn read_multi_object(b: Vec<u8>) -> std::io::Result<Vec<Vec<u8>>> {
...
impl GroupMember {
    pub(crate) fn decode(b: Vec<u8>) -> std::io::Result<GroupMember> {
...
impl Group {
    ...
    //用于get_group_list
    //只有基础信息
    pub(crate) fn decode_base(b: Vec<u8>) -> std::io::Result<Group> {
        let mut b = Cursor::new(b);
        Ok(Group {
            group_id: b.read_i64::<BigEndian>()?,
    ...
    //含有基础和群人数信息
    pub(crate) fn decode(b: Vec<u8>) -> std::io::Result<Group> {
        let mut b = Cursor::new(base64::decode(&b).unwrap());
        Ok(Group {
            group_id: b.read_i64::<BigEndian>()?,
            group_name: b.read_string()?,
...
impl User {
    ...
    pub(crate) fn decode(b: Vec<u8>) -> std::io::Result<User> {
        let mut b = Cursor::new(base64::decode(&b).unwrap());
        Ok(User {
            user_id: b.read_i64::<BigEndian>()?,
            nickname: b.read_string()?,
        ...
        pub(crate) fn to_default(&self) -> String {
            ...
            match self {
                ...
                Image::Http(s) => {
                    ...
                    //最多只能重定向10次，并在5秒之后超时
                    match reqwest::blocking::Client::builder().timeout(Duration::from_secs(5)).build().unwrap().get(s.as_str()).send() {
                        Ok(mut r) => {
        ...
        pub(crate) async fn to_default_async(&self) -> String {
            ...
            match self {
                ...
                Image::Http(s) => {
                    ...
                    let mut b: Vec<u8> = Vec::new();
                    //最多只能重定向10次，并在5秒之后超时
                    match reqwest::Client::builder().timeout(Duration::from_secs(5)).build().unwrap().get(s.as_str()).send().await {
                        Ok(mut r) => {
